...
import me.limeglass.fawesk.utils.annotations.DetermineSingle;
import me.limeglass.fawesk.utils.annotations.Events;
import me.limeglass.fawesk.utils.annotations.Multiple;
...
public abstract class FaweskExpression<T> extends SimpleExpression<T> implements DataChecker {
	...
	public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parser) {
		if (getClass().isAnnotationPresent(Events.class)) {
			if (!contains()) {
				Fawesk.debugMessage("The expression `" + getClass().getSimpleName() + "` can't be used in the event: " + ScriptLoader.getCurrentEventName() + "it can only be used in: " + Arrays.toString(getClass().getAnnotation(Events.class).value()));
				return false;
			}
		}
