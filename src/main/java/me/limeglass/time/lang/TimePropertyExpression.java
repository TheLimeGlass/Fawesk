...
public abstract class TimePropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	public String toString(Event event, boolean debug) {
		...
		return Time.getInstance().getNameplate() + getClass().getSimpleName() + ": the " + getPropertyName() + " (of|from) " + getExpr().toString(event, debug);
	}
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			Time.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
