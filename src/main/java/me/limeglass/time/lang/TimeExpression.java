...
import me.limeglass.time.utils.annotations.DetermineSingle;
import me.limeglass.time.utils.annotations.Events;
import me.limeglass.time.utils.annotations.Multiple;
import me.limeglass.time.utils.annotations.Settable;
import me.limeglass.time.utils.annotations.Single;

public abstract class TimeExpression<T> extends SimpleExpression<T> implements DataChecker {
	...
	public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parser) {
		if (getClass().isAnnotationPresent(Events.class)) {
			if (!contains()) {
				Time.debugMessage("The expression `" + getClass().getSimpleName() + "` can't be used in the event: " + ScriptLoader.getCurrentEventName() + "it can only be used in: " + Arrays.toString(getClass().getAnnotation(Events.class).value()));
				return false;
			}
		}
	...
	public String toString(Event event, boolean debug) {
		...
		if (event != null) Time.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
	...
	public Boolean isNull(Event event, int... index) {
		for (int i : index) {
			if (isNull(event, expressions, i)) return true;
		}
		return false;
	}
