...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Time.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Time.getConfiguration("syntax").set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enumType.getEnumConstants());
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Time.getInstance().getConfig().getString("syntaxDebug"))) Time.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Time.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Time.getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Time.save("syntax");
	}
}