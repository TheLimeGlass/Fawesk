...
import me.limeglass.time.lang.TimeParser;

public class LangEnumParser<T extends Enum<T>> extends TimeParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			Time.getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			Time.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
		Time.save("syntax");
	}
