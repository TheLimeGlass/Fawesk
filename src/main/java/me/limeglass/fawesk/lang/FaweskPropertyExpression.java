...
public abstract class FaweskPropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			Fawesk.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
