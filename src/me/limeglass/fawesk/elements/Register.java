...
import me.limeglass.fawesk.Metrics;
import me.limeglass.fawesk.Syntax;
import me.limeglass.fawesk.Fawesk;
import me.limeglass.fawesk.utils.EnumClassInfo;
import me.limeglass.fawesk.utils.TypeClassInfo;
import me.limeglass.fawesk.utils.annotations.Disabled;
import me.limeglass.fawesk.utils.annotations.ExpressionProperty;
...
import me.limeglass.fawesk.utils.annotations.RegisterEnum;
import me.limeglass.fawesk.utils.annotations.RegisterType;
import me.limeglass.fawesk.utils.annotations.User;
...
public class Register {
	...
	public static void register() {
		try {
			...
			File file = (File) method.invoke(Fawesk.getInstance());
			...
			c : for (Enumeration<JarEntry> jarEntry = addon.entries(); jarEntry.hasMoreElements();) {
				...
				if (name.startsWith(Fawesk.getInstance().getPackageName()) && name.endsWith(".class")) {
		...
		run : for (Class clazz : classes) {
			if (!clazz.isAnnotationPresent(Disabled.class)) {
				...
				if (clazz.isAnnotationPresent(Patterns.class)) {
					...
					String[] values = new String[]{Fawesk.getInstance().getNameplate() + input1 + " " + properties[1] + " (of|from)" + additions + "%" + properties[0] + "%", Fawesk.getInstance().getNameplate() + input2 + "%" + properties[0] + "%['s]"  + additions.replace("[the] ", "") + properties[1]};
					syntax = Syntax.register(clazz, values);
					if (syntax == null) Fawesk.debugMessage("&cThere was an issue registering the syntax for " + clazz.getName() + ". Make sure that the SyntaxToggles.yml is set for this syntax.");
				} else {
					continue run;
				}
				...
				if (syntax != null) {
					if (Effect.class.isAssignableFrom(clazz)) {
						...
						Fawesk.debugMessage("&5Registered Effect " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
					} else if (Condition.class.isAssignableFrom(clazz)) {
						Skript.registerCondition(clazz, syntax);
						Fawesk.debugMessage("&5Registered Condition " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						...
						try {
							...
							Fawesk.debugMessage("&5Registered Expression " + type.toString() + " " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						} catch (IllegalAccessException | IllegalArgumentException | InstantiationException e) {
							Fawesk.consoleMessage("&cFailed to register expression " + clazz.getCanonicalName());
							e.printStackTrace();
						}
					}
				}
	...
	public static void metrics(Metrics metrics) {
		...
		Fawesk.debugMessage("Metrics registered!");
	}
}